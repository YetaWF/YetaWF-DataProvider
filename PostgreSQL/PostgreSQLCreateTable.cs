...
namespace YetaWF.DataProvider.PostgreSQL {
    ...
    internal class PostgreSQLCreate {

        public int IdentitySeed { get; private set; }
        public bool Logging { get; private set; }
        public List<LanguageData> Languages { get; private set; }

        public PostgreSQLCreate(List<LanguageData> languages, int identitySeed, bool logging) {
            Languages = languages;
...
//$$$        List<ForeignKey> SavedNewKeys = null;
        ...
        public bool CreateTable(PostgreSQLCache.DBEntry db, string schema, string tableName, string key1Name, string key2Name, string identityName, List<PropertyData> propData, Type tpProps,
            ...
            try {
                ...
                if (db.Tables.ContainsKey(tableName)) {
                    ...
                    if (newTab.Schema != schema)
                        throw new InternalError("Can't change table {0} schema {1} to {2}", tableName, newTab.Schema, schema);
                ...
                bool hasSubTable = AddTableColumns(db, schema, updatingTable, origColumns, origIndexes, newTab, tableName, key1Name, key2Name, identityName, propData, tpProps, "", true, columns, errorList, savedColumnsWithConstraints, SubTable: SubTable);
                ...
                if (SiteSpecific) {
                    Column newColumn = MakeColumn(origColumns, newTab, PostgreSQLBase.SiteColumn);
                ...
                if (SubTable) { // for replication
                    Column newColumn = MakeColumn(origColumns, newTab, PostgreSQLBase.IdentityColumn);
                    newColumn.DataType = Microsoft.SqlServer.Management.Smo.DataType.Int;
                    newColumn.Nullable = false;
                    ...
                    Index index = MakeIndex(origIndexes, newTab, "K_" + tableName + "_" + PostgreSQLBase.IdentityColumn, PostgreSQLBase.IdentityColumn);
                ...
                if (ForeignKeyTable != null) {
                    if (SubTable) {
                        ...
                        Column newColumn = MakeColumn(origColumns, newTab, PostgreSQLBase.SubTableKeyColumn);
                        newColumn.DataType = Microsoft.SqlServer.Management.Smo.DataType.Int;
                        newColumn.Nullable = false;

                        Index index = MakeIndex(origIndexes, newTab, "K_" + tableName + "_" + PostgreSQLBase.SubTableKeyColumn, PostgreSQLBase.SubTableKeyColumn);
                        index.IndexKeyType = Microsoft.SqlServer.Management.Smo.IndexKeyType.None;
                        ...
                        MakeForeignKey(newTab, "FK_" + tableName + PostgreSQLBase.SubTableKeyColumn + "_" + ForeignKeyTable + "_" + identityName, PostgreSQLBase.SubTableKeyColumn, ForeignKeyTable, identityName);
                    } else {
        ...
        private Index MakeIndex(List<string> origIndexes, Table table, string indexName, string columnName, string ColumnName2 = null, bool AddSiteKey = false) {
            ...
            if (table.Indexes.Contains(indexName)) {
                ...
                if (AddSiteKey && !newIndex.IndexedColumns.Contains(PostgreSQLBase.SiteColumn))
                ...
                if (AddSiteKey)
                    newIndex.IndexedColumns.Add(new IndexedColumn(newIndex, PostgreSQLBase.SiteColumn));
            }
        ...
        private ForeignKey MakeForeignKey(Table table, string fkName, string column, string referencedTable, string referencedColumn, bool AddSiteKey = false) {
            ...
            if (table.ForeignKeys.Contains(fkName)) {
                ...
                if (AddSiteKey && !fk.Columns.Contains(PostgreSQLBase.SiteColumn))
                ...
                if (AddSiteKey) {
                    ForeignKeyColumn fkcSite = new ForeignKeyColumn(fk, PostgreSQLBase.SiteColumn, PostgreSQLBase.SiteColumn);
        ...
        private bool AddTableColumns(Database db, string schema, bool updatingTable, List<string> origColumns, List<string> origIndexes, Table newTab,
                ...
                if (pi.CanRead && pi.CanWrite && !prop.HasAttribute("DontSave") && !prop.CalculatedProperty && !prop.HasAttribute(Data_DontSave.AttributeName)) {
                    ...
                    if (colName == key1Name || colName == key2Name || !columns.Contains(colName)) {
                        ...
                        if (prop.Name == identityName) {
                            ...
                            bool success = CreateTable(db, schema, subTableName, PostgreSQLBase.SubTableKeyColumn, null,
                            ...
                            AddTableColumns(db, schema, updatingTable, origColumns, origIndexes, newTab, tableName, null, null, identityName, subPropData, pi.PropertyType, prefix + prop.Name + "_", SubTable, columns, errorList, savedColumnsWithConstraints);
                        } else
        ...
        public void DropAllTables(Database db, string schema) {
        ...
        public bool DropTable(Database db, string schema, string tableName, List<string> errorList) {
            foreach (Table table in db.Tables) {
        ...
        public bool DropSubTables(Database db, string schema, string tableName, List<string> errorList) {
            bool status = true;
            ...
            foreach (Table table in tables) {
                if (table.Name.StartsWith(subtablePrefix))
                    if (!DropTable(db, schema, table.Name, errorList))
                        status = false;
            }
            return status;
        }
